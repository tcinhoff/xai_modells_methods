...
@app.callback(
    Output(
        "dataset-split-status", "children"
    ),  # Ein Ausgabeelement für Statusmeldungen
    [
...
def split_and_save_datasets(
    train_start, train_end, test_start, test_end, target_column, date_range_info
):
    ...
    if "," in date_range_info:
        ...
        min_date_str, max_date_str = date_range_info.split(",")
        df = pd.read_csv(
            PATHS["train_test_data_path"]
        )  # Angenommen, dies ist der Pfad zum Gesamtdataset
        df["date"] = pd.to_datetime(df["date"])

        # Aufteilung in Trainings- und Testdaten
        train_df = df[
            (df["date"] >= pd.to_datetime(train_start))
            & (df["date"] <= pd.to_datetime(train_end))
        ]
        test_df = df[
            (df["date"] >= pd.to_datetime(test_start))
            & (df["date"] <= pd.to_datetime(test_end))
        ]

        # Speichern der aufgeteilten Daten
        train_df.to_csv(PATHS["train_data_path"], index=False)
        test_df.drop(columns=[target_column]).to_csv(
            PATHS["test_data_path"], index=False

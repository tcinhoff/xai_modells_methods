...
def get_lime_component(selected_model, point_index):
    lime_html = perform_lime_analysis(selected_model, point_index)
    return html.Iframe(srcDoc=lime_html, style={"width": "100%", "height": "600px"})


def perform_lime_analysis(selected_model, point_index):
    ...
    exp = explainer.explain_instance(test_data.iloc[point_index].values, model.predict)

...
def get_shap_waterfall_component(selected_model, point_index):
    if "SHAP" in MODELS[selected_model]["compatible_methods"]:
        image_src = create_shap_waterfall_plot(selected_model, point_index)
        return html.Img(src=image_src, style={"width": "100%", "height": "500px"})
    else:
        return "SHAP analysis is not available for this model."


def create_shap_waterfall_plot(selected_model, point_index):
    ...
    return create_shap_waterfall_image(shap_values, point_index)


def create_shap_waterfall_image(shap_values, point_index):
    plt.figure(figsize=(10, 6))
    shap.plots.waterfall(shap_values[point_index], max_display=14, show=False)
